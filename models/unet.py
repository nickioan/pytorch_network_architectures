...
from .common import Bottleneck, BasicBlock, DownBlock, UpBlock, conv1


class UNet(nn.Module):

    def __init__(self, layers, k1=7, k=3, cin=3, nk=64, skip=True, act='prelu',
                 down='max', up='nearest', G=2, nrm='identity', bias=False,
    ...
    def _make_enc(self, layers, chs):
        ...
        for i in range(len(layers)):
            ...
            for j in range(layers[i]):
                ...
                l = BasicBlock(ch, alpha=self.G, k=self.k, act=self.act,
    ...
    def _make_dec(self, layers, chs):
        ...
        for i in range(len(layers)):
            ...
            for j in range(layers[i]):
                ...
                l = BasicBlock(ch, k=self.k, alpha=self.G, act=self.act,
                               nrm=self.nrm, bias=self.bias, skip=self.skip,
                               depth=i, L=layers[i])
                layer.append(l)
