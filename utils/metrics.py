...
def accuracy_segmentation(y_pred,y):
    smooth=1e-2
    dims=(1,2,3)
    intersection = th.sum(y*y_pred,dim=dims)
    #print(y_pred.size())
    print("Volume Prediction")
    print(th.sum(y_pred,dims))
    print("Ground Truth Volume")
    print(th.sum(y,dims))
    print("Intersection")
    print(intersection)
    union = th.sum(y,dim=dims) + th.sum(y_pred,dim=dims) - intersection
